...
export function addWidgets(opts: Options, node: Selection<Element, HierarchyNode<PedigreeDatasetNode>, HTMLElement, any>, showWidgets: boolean = true) {
	...
	// Rectangle used to highlight the selected person
	node.filter(function (d) {
		return !(d.data.hidden && !opts.DEBUG) && d.data.isUiSelected;
	})
		.append("rect")
		.attr("class", 'indi_rect')
		.attr("rx", 6)
		.attr("ry", 6)
		.attr("x", function () { return -1 * opts.symbol_size; })
		.attr("y", function () { return -opts.symbol_size; })
