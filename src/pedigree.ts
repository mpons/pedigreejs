...
import {init_zoom, scale_to_fit, zoomToSelected} from './zoom.ts';
...
import {
    balanceTree, computeChildrenNumber,
    computeDistancesFromProband,
    computeLineageHeights,
    computeSiblingsNumber, computeSubtreeWidth,
    getNodeWidthMultiplier
} from "./utils.ts";
...
export function build(options: Options) {
    const defaults = { // defaults
        ...
        height: 400,
        symbol_size: 35,
        horizontalSpacingFactor: 1.3,
        verticalSpacingFactor: 1.8,
    ...
    utils.computeLineageHeights(opts.dataset)
    utils.computeSubtreeWidth(opts.dataset)
    utils.computeChildrenNumber(opts.dataset)
    ...
    let treemap = tree<PedigreeDatasetNode>()
        .separation((a: HierarchyNode<PedigreeDatasetNode>, b: HierarchyNode<PedigreeDatasetNode>) => {
            const aMultiplier = getNodeWidthMultiplier(options, a)
    ...
    allNodes.filter(function (d) {
        ...
        .style("stroke", function (d) {
            return d.data.age !== undefined && d.data.yob !== undefined && !d.data.exclude ? "#303030" : "grey";
        })
        .style("stroke-width", function (d) {
            return d.data.age !== undefined && d.data.yob !== undefined && !d.data.exclude ? ".3em" : ".1em";
        })
    ...
    //
    addWidgets(opts, allNodes, opts.showWidgets);
...
function initZoomThenSelect(opts: Options) {
    init_zoom(opts, select("#" + opts.targetDiv).select("svg"));
    setTimeout(() => {
        //scale_to_fit(opts);
        zoomToSelected(opts);
    }, 0);
...
function drawConnectionLinesBetweenPartners(
    ...
    ped.selectAll(".partner")
        ...
        .attr('d', function (d: D3PartnerLink, _i) {
            ...
            if (node1?.data.hidden && node2?.data.hidden) {
                return
            }
...
function drawProbandArrow(
    ...
    if (probandIdx) {
        ...
        ped.append("line")
            .attr("x1", probandX - symbolSize*2)
            .attr("y1", probandY + symbolSize)
            .attr("x2", probandX - (symbolSize / 1.4))
            .attr("y2", probandY + (symbolSize / 4))
            .attr("stroke-width", symbolSize/20)
            .attr("stroke", "black")
        ...
        ped.append("line")
            ...
            .attr("x2", probandX - (symbolSize / 1.4))
            .attr("y2", probandY + (symbolSize / 4))
            .attr("stroke-width", symbolSize/10)
            .attr("stroke", "black")
    }
}
...
export function checkPartnerLinkClashes(
    ...
    // Calculate label width based function
    const getLabelWidth = (node?: HierarchyNode<PedigreeDatasetNode>) => {
        if (!node) return 1.0;

        // Base size is the symbol size
        const baseWidth = 1.0;

        // If node has a display_name, adjust based on length
        if (node.data.display_name) {
            // Longer names need more space
            return Math.max(baseWidth, node.data.display_name.length / 10);
        }

        // Check for gene test results
        if (node.data.hasOwnProperty('gene_test')) {
            return 1.5; // Gene tests need more space
        }
    ...
    return flattenNodes.map((bnode) => {
        ...
        // Adjust the width based on label size
        const nodeWidth = getLabelWidth(bnode);
