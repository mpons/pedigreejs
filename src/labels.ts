...
export function addLabels(opts: Options, node: Selection<SVGGElement, HierarchyPointNode<PedigreeDatasetNode>, SVGElement, PedigreeDatasetNode>) {
    ...
    addLabel(opts,
        node,
        (d) => {
            const name = 'display_name' in d.data ? d.data.display_name : ''
            return -opts.symbol_size/2 - ((name.length - 3) * 2)
        },
        function (d) {
            return ypos(d, ['display_name'], font_size);
        },
    ...
    for (let i = 0; i < opts.diseases.length; i++) {
        ...
        let displayName = opts.diseases[i].display_name
        ...
        addLabel(opts, node, -opts.symbol_size,
            ...
            function (d) {
                let dis = displayName !== undefined ? displayName : disease.replace('_', ' ').replace('cancer', 'ca.')

                if (disease === 'other_cancer' && d.data.other_cancer_name) {
                    dis = d.data.other_cancer_name
                }
...
function addLabel(
    opts: Options,
    node: Selection<SVGGElement, HierarchyNode<PedigreeDatasetNode>, SVGElement, PedigreeDatasetNode>,
    ...
    let fontWeight = opts.font_weight
