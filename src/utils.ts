...
export function balanceTree(dataset: PedigreeDatasetNode[]) {
	const probandPerson = getProband(dataset)
	if (!probandPerson) {
		throw 'Proband not found'
	}
	...
	if (!father || !mother) {
		return; // Cannot balance if either parent is missing
	}

	let fathersHeight = getAncestorsHeight(dataset, father)
	let mothersHeight = getAncestorsHeight(dataset, mother)

	console.log('fathersHeight', fathersHeight)
	console.log('mothersHeight', mothersHeight)

	// Determine which side needs adjustment and how many generations to add
	const heightDifference = Math.abs(fathersHeight - mothersHeight)
	if (heightDifference === 0) {
		return; // Tree is already balanced
	}

	const sideToAdjust = fathersHeight < mothersHeight ? father : mother

	// Add hidden ancestors to the shorter branch
	let currentNode = sideToAdjust
	let familyId = currentNode.famid
	let i = 0;
	while ( i < heightDifference) {

		if (currentNode.mother) {
			const motherNode = getPedigreeNodeByName(dataset, getName(currentNode.mother))
			if (motherNode) {
		...
		// Create hidden parents for the current node
		const hiddenMother: PedigreeDatasetNode = {
			...
			ashkenazi: false,
			parent: null,
			noparents: i === heightDifference - 1 // Mark as having no parents at the top level
		}
		...
		i++
		console.log('add hidden node to ', currentNode)
		// Add the hidden nodes to the dataset
		dataset.push(hiddenMother);
		dataset.push(hiddenFather);

		// Set these hidden nodes as the parents of the current node
		currentNode.mother = hiddenMother.name;
		currentNode.father = hiddenFather.name;
		currentNode.noparents = false;

		// Move up to the hidden father for next iteration
		currentNode = hiddenMother;
	}

	console.log('dataset after balance tree', dataset)
	return dataset;
}

export function computeChildrenNumber(dataset: PedigreeDatasetNode[]) {
	// Initialize all nodes with undefined childrenNumber
	dataset.forEach(person => {
		person.childrenNumber = undefined;
	});
	...
	dataset.forEach(person => {
		...
		// Set childrenNumber to the count of children
		person.childrenNumber = children.length;
	});
}

function getAncestorsHeight(dataset: PedigreeDatasetNode[], startingNode: PedigreeDatasetNode): number {
	// Set of already visited nodes to prevent infinite loops in case of circular references
	const visitedNodes = new Set<string>();

	function getHeightRecursive(node: PedigreeDatasetNode | undefined): number {
		if (!node || visitedNodes.has(node.name) || node.noparents || node.top_level) {
			return 0;
		}
		...
		// Find parent nodes
		const motherName = getName(node.mother);
		const fatherName = getName(node.father);

		if (!motherName && !fatherName) {
			return 0;
		}

		const motherNode = motherName ? getPedigreeNodeByName(dataset, motherName) : undefined;
		const fatherNode = fatherName ? getPedigreeNodeByName(dataset, fatherName) : undefined;

		// Get height from both parents and take the maximum
		const motherHeight = motherNode ? 1 + getHeightRecursive(motherNode) : 0;
		const fatherHeight = fatherNode ? 1 + getHeightRecursive(fatherNode) : 0;

		return Math.max(motherHeight, fatherHeight);
	}

	return getHeightRecursive(startingNode);
}
...
/**
 * Computes the subtree width for each node in the dataset. The subtree width is the maximum
 * number of siblings or partners at any level in the node's subtree (including the node itself).
 *
 * @param dataset Array of pedigree dataset nodes
 */
export function computeSubtreeWidth(dataset: PedigreeDatasetNode[]) {
	// Initialize all nodes with undefined subtreeWidth
	dataset.forEach(person => {
		person.subtreeWidth = undefined;
	});

	// Process nodes in bottom-up order (from leaves to root)
	// First, identify nodes without children (leaves)
	const nodesByDepth: Record<number, PedigreeDatasetNode[]> = {};
	const maxDepth = dataset.reduce((max, person) => {
		const depth = getDepth(dataset, person.name);
		if (!nodesByDepth[depth]) {
			nodesByDepth[depth] = [];
		}
		nodesByDepth[depth].push(person);
		return Math.max(max, depth);
	}, 0);

	// Process from leaves up to root (bottom-up)
	for (let depth = maxDepth; depth >= 1; depth--) {
		if (nodesByDepth[depth]) {
			nodesByDepth[depth].forEach(person => {
				calculateSubtreeWidth(dataset, person);
			});
		}
	}
}
...
/**
 * Calculates the subtree width for a single node.
 *
 * @param dataset Array of pedigree dataset nodes
 * @param person The node to calculate subtree width for
 * @returns The calculated subtree width
 */
function calculateSubtreeWidth(dataset: PedigreeDatasetNode[], person: PedigreeDatasetNode): number {
	// If already calculated, return the value
	if (person.subtreeWidth !== undefined) {
		return person.subtreeWidth;
	}
	...
	// Get siblings
	const siblings = getSiblings(dataset, person);

	// Calculate width at this level (siblings + self + partners)
	const levelWidth = siblings.length + 1 + partners.length;

	// If no children, width is just this level
	if (children.length === 0) {
		person.subtreeWidth = levelWidth;
		return levelWidth;
	}

	// Calculate widths of all children's subtrees
	const childrenWidths = children.map(child => calculateSubtreeWidth(dataset, child));

	// Find the maximum width among children
	const maxChildWidth = Math.max(...childrenWidths);

	// Subtree width is the maximum of this level's width and children's maximum width
	person.subtreeWidth = Math.max(levelWidth, maxChildWidth);
	return person.subtreeWidth;
}


export function computeLineageHeights(dataset: PedigreeDatasetNode[]) {
	// Initialize all nodes with undefined lineageHeight
	dataset.forEach(person => {
		person.lineageHeight = undefined;
	});

	// Process each node in the dataset
	dataset.forEach(person => {
		if (person.lineageHeight === undefined) {
			// Calculate lineage height for this node
			person.lineageHeight = calculateLineageHeight(dataset, person);
		}
	});
}

/**
 * Calculates the lineage height for a single node by counting generations
 * above (ancestors) and below (descendants) plus 1 for the node itself.
 *
 * @param dataset Array of pedigree dataset nodes
 * @param person The node to calculate lineage height for
 * @returns The total lineage height
 */
function calculateLineageHeight(dataset: PedigreeDatasetNode[], person: PedigreeDatasetNode): number {
	...
	function getDescendantsHeight(node: PedigreeDatasetNode): number {
		...
		const children = getAllChildren(dataset, node);

		if (children.length === 0) {
			return 0;
		}

		// Find the maximum height among all children paths
		let maxChildHeight = 0;
		children.forEach(child => {
			const childHeight = 1 + getDescendantsHeight(child);
...
export function horizontalOverlap(opts: Options, nodes: HierarchyNode<PedigreeDatasetNode>[], xnew: number, depth: number, exclude_names: string[]) {
	const horizontalOverlapToleranceMargin = 0.5
...
export function get_tree_dimensions(opts: Options) {
	...
	// Calculate label length factor
	const getLabelFactor = (person: PedigreeDatasetNode) => {
		if (!person.display_name) return 1;

		// Increase factor based on display name length
		const nameFactor = Math.max(1, person.display_name.length / 8);

		// Additional space for gene tests or other attributes
		const attributesFactor = person.hasOwnProperty('gene_test') ? 1.3 : 1;

		return nameFactor * attributesFactor;
	};
	...
	for(let i=0; opts.dataset !== undefined && i < opts.dataset.length; i++) {
		...
		// Add label length factor to the score
		const labelFactor = getLabelFactor(opts.dataset[i]);

		// score based on no. of children, label length, and if parent defined
		let score = (1 + (children.length > 0 ? 0.55 + (children.length * 0.25) : 0) +
			(opts.dataset[i].father ? 0.25 : 0)) * labelFactor;
	...
	return {
		width: tree_width * opts.horizontalSpacingFactor,
		height: tree_height * opts.verticalSpacingFactor
	};
}
...
export function buildPerson(id: string, displayName: string, sex: Sex, father?: string, mother?: string, topLevel: boolean = false, noParents: boolean = false): PedigreeDatasetNode {
	return {
		name: id,
		sex: sex,
		top_level: topLevel,
...
export function getNodeWidthMultiplier(
	opts: Options,
	node: HierarchyNode<PedigreeDatasetNode>
): number {
	// Base multiplier (symbol itself)
	let widthMultiplier = 1.0;

	// Check if the node has a display name
	if (node.data.display_name) {
		// Estimate width based on character count (adjust multiplier as needed)
		const charWidth = 0.5; // Character width relative to symbol size
		const nameWidthMultiplier = Math.max(1, (node.data.display_name.length * charWidth) / 3);
		widthMultiplier = Math.max(widthMultiplier, nameWidthMultiplier);
	}

	// Check for gene test results that need to be displayed
	const geneTests = [
	...
	// Add width for gene tests
	if (geneTestCount > 0) {
		widthMultiplier = Math.max(widthMultiplier, 1 + (geneTestCount * 0.3));
	}

	// Check for cancer diagnoses info
	const cancerTypes = [
		'breast_cancer_diagnosis_age', 'breast_cancer2_diagnosis_age',
		'ovarian_cancer_diagnosis_age', 'prostate_cancer_diagnosis_age',
		'pancreatic_cancer_diagnosis_age', 'other_cancer_diagnosis_age'
	];

	let cancerInfoCount = 0;
	...
	// Add width for cancer diagnoses
	if (cancerInfoCount > 0) {
		widthMultiplier = Math.max(widthMultiplier, 1 + (cancerInfoCount * 0.4));
	}
	...
	if (node.data.notes) {
		...
		const longestLineLength = Math.min(
			...Array.from({length: lineCount}, (_, i) =>
				words.slice(i * wordsPerLine, (i + 1) * wordsPerLine).join(' ').length
			)
		);

		// Add width for notes
		const notesWidthMultiplier = Math.max(1, (longestLineLength * 0.4) / 3);
		widthMultiplier = Math.max(widthMultiplier, notesWidthMultiplier);
	}

	// Add width for BC pathology info
	...
	if (pathologyCount > 0) {
		widthMultiplier = Math.max(widthMultiplier, 1 + (pathologyCount * 0.25));
	}

	// Check for other displayed labels
	if (opts.labels && Array.isArray(opts.labels)) {
		// Add additional width for displayed labels
		for (const label of opts.labels.flat()) {
			if (node.data[label] && ['age', 'yob', 'alleles'].includes(label)) {
				widthMultiplier += 0.3;
			}
		}
	}
